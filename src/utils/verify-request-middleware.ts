...
import { NotAuthorizedError } from '@mohamedramadan14/freelance-shared';
import { config } from '@authentication/config';

const tokens: string[] = ['auth', 'seller', 'gig', 'search', 'buyer', 'message', 'order', 'review'];

export function verifyGatewayRequest(req: Request, _res: Response, next: NextFunction): void {
  console.log('FROM verifyGatewayRequest()');
  console.log(req.headers?.gatewaytoken);

  if (!req.headers?.gatewaytoken) {
    throw new NotAuthorizedError('Invalid request NO TOKEN', 'verifyGatewayRequest() method: Request not coming from api gateway');
  }
  const token: string = req.headers?.gatewaytoken as string;
  if (!token) {
    throw new NotAuthorizedError('Invalid request NO TOKEN 2', 'verifyGatewayRequest() method: Request not coming from api gateway');
  }
  ...
  console.log('FROM verifyGatewayRequest() SUCCESS');
  next();
}
